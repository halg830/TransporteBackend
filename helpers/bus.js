...
const helpersBus = {
  ...
  existePlaca: async (placa, req) => {
    const placaRegExp = new RegExp(
      `^${await helpersGeneral.quitarTildes(placa)}$`,
      "i"
    );
  ...
  numeroEmpresa: async (empresa, req) => {
    ...
    const empresaRegExp = new RegExp(
      `^${await helpersGeneral.quitarTildes(sin)}$`,
      "i"
    );
    const bus = await Bus.findOne({
      $and: [{ empresa: { $regex: empresaRegExp } }, { numero }],
    });
