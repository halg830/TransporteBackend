...
router.post(
  ...
  [
    ...
    check("cedula", "La cedula debe tener 10 digitos").isLength({
      max: 10,
      min: 10,
    }),
    check("cedula", "La cedula ya esta registrada").custom(
      helpersVendedor.existeCedula
    ),
    check("telefono", "El numero es obligatorio").notEmpty(),
    check("telefono").custom(helpersGeneral.verificarEspacios),
    check("telefono", "El numero debe tener 10 digitos").isLength({
      max: 10,
      min: 10,
    }),
    check("usuario", "El usuario es obligatorio").notEmpty(),
    check("usuario").custom(helpersGeneral.verificarEspacios),
    check("usuario", "El usuario debe tener 6 digitos o más").isLength({
      min: 6,
    }),
    check("usuario", "El nombre de usuario no esta disponible").custom(
      helpersVendedor.existeUsuario
    ),
...
router.put(
  "/inactivar/:id",
  [check("id").custom(helpersVendedor.noInactivarAdmin), validarCampos, validarJWT],
